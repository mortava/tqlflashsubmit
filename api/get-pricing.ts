...
function dscrRangeFromNumeric(val: number): string {
  if (val >= 1.250) return '>=1.250'
  if (val >= 1.150) return '1.150-1.249'
  if (val >= 1.000) return '1.00-1.149'
  if (val >= 0.750) return '0.750-0.999'
  if (val >= 0.500) return '0.500-0.749'
  return 'noRatio'
}

function mapDSCRRatio(dscrRatio: string, dscrValue?: number, dscrManualInput?: string): number {
  const map: Record<string, number> = {
    '>=1.250': 1,
    '1.150-1.249': 2,
    '1.00-1.149': 3,
  ...
  // Primary: use the range string directly
  if (dscrRatio && map[dscrRatio]) return map[dscrRatio]
  // Fallback: compute range from numeric dscrValue or dscrManualInput
  const numericVal = dscrValue || parseFloat(dscrManualInput || '') || 0
  if (numericVal > 0) {
    const computedRange = dscrRangeFromNumeric(numericVal)
    console.log(`[DSCR fallback] dscrRatio="${dscrRatio}" missing/invalid, computed from numeric ${numericVal} → "${computedRange}"`)
    return map[computedRange] || 3
  }
  return 3 // default to 1.00-1.149
}
...
function buildLOXmlFormat(formData: any): string {
  let loanAmount = Number(formData.loanAmount) || 400000
  let propertyValue = Number(formData.propertyValue) || 500000
  ...
  const origLtv = (loanAmount / propertyValue) * 100

  // Small-balance override: ONLY for Investment + DSCR
  // ML won't price very small loans, so inflate to minimums while preserving LTV
  const isInvestmentDSCR = isDSCR && (formData.occupancyType === 'investment')
  if (isInvestmentDSCR) {
    // Loan amount floor: $105,000 — scale property value to preserve original LTV
    if (loanAmount < 105000) {
      loanAmount = 105000
      if (origLtv > 0) {
        propertyValue = Math.round(105000 / (origLtv / 100))
      }
    }
    // Property value floor: $150,000 for ML
    if (propertyValue < 150000) {
      propertyValue = 150000
    }
  }

  // ── DSCR Edge-Case Nudge ──────────────────────────────────
  // ML won't price DSCR at exactly 1.000 boundary. Nudge values so ML sees
  // DSCR slightly above 1.000:
  //   • If user selected DSCR 1.000 → pass 1.001 to ML
  //   • If rent == payment (e.g. both 5000) → bump rent by $5 (5000 → 5005)
  if (isDSCR) {
    const dscrNumeric = parseFloat(formData.dscrValue) || parseFloat(formData.dscrManualInput) || 0
    if (dscrNumeric > 0 && dscrNumeric <= 1.000) {
      formData = { ...formData, dscrValue: '1.001', dscrManualInput: '1.001' }
      console.log(`[DSCR nudge] User DSCR ${dscrNumeric} → 1.001 for ML`)
    }
    const rent = Number(String(formData.grossRent || '0').replace(/,/g, ''))
    const payment = Number(String(formData.presentHousingExpense || '0').replace(/,/g, ''))
    if (rent > 0 && payment > 0 && rent <= payment) {
      formData = { ...formData, grossRent: String(payment + 5) }
      console.log(`[DSCR nudge] Rent ${rent} <= Payment ${payment}, bumped rent to ${payment + 5}`)
    }
  }
  ...
  <loan>
    ...
    ${isDSCR ? `<field id="aDSCR %">${mapDSCRRatio(formData.dscrRatio, parseFloat(formData.dscrValue) || 0, formData.dscrManualInput)}</field>` : ''}
    ${isDSCR ? `<field id="aOccupancyRate">100</field>` : ''}
    ${isDSCR ? `<field id="sSpGrossRentPe">${Number(String(formData.grossRent || '5000').replace(/,/g, '')) || 5000}</field>` : ''}
    ${isDSCR ? `<field id="sPresOHExpPe">${Number(String(formData.presentHousingExpense || '5000').replace(/,/g, '')) || 5000}</field>` : ''}
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Ensure dscrRatio is set from numeric value if missing/invalid (handles 1.0, 1.00, 1.000 inputs)
    if (isDSCRRequest) {
      const validRanges = ['>=1.250', '1.150-1.249', '1.00-1.149', '0.750-0.999', '0.500-0.749', 'noRatio']
      if (!formData.dscrRatio || !validRanges.includes(formData.dscrRatio)) {
        const numericDscr = parseFloat(formData.dscrValue) || parseFloat(formData.dscrManualInput) || 0
        if (numericDscr > 0) {
          formData.dscrRatio = dscrRangeFromNumeric(numericDscr)
          console.log(`[DSCR] Normalized dscrRatio from numeric ${numericDscr} → "${formData.dscrRatio}"`)
        }
      }
    }
    ...
    if (responseText.includes('status=&quot;Error&quot;') || responseText.includes('status="Error"')) {
      ...
      // Try additional patterns to extract ML error message
      const altMatch = responseText.match(/message=["&]quot;([^"&]+)/) || responseText.match(/Error.*?>([\s\S]{1,200}?)</)
      const mlError = errorMatch?.[1] || altMatch?.[1]?.trim() || 'Unknown pricing error'
      console.error('[ML] Error response (first 500 chars):', responseText.substring(0, 500))
      return res.json({ success: false, error: mlError, debug: { responseSnippet: responseText.substring(0, 300) } })
    }
    ...
    eligiblePrograms.forEach((p: any) => {
      ...
      p.rateOptions.forEach((ro: any) => {
        ...
        ro.adjustments = ro.adjustments.filter((adj: any) => {
          ...
          // Keep DSCR ratio tier adjustments (e.g. "DSCR >=1.250") — they are real LLPAs
    ...
    // DSCR small-balance manual adjustment: -1.125 price hit for loans under $100K
    const origLoanAmount = Number(formData.loanAmount) || 400000
    const isSmallBalanceDSCR = isDSCRRequest && origLoanAmount >= 75000 && origLoanAmount <= 99999
    if (isSmallBalanceDSCR) {
      eligiblePrograms.forEach((p: any) => {
        if (!p.rateOptions) return
        p.rateOptions.forEach((ro: any) => {
          if (!ro.adjustments) ro.adjustments = []
          ro.adjustments.push({
            description: 'SMALL BALANCE DSCR (75K-99K)',
            amount: -1.375,
            rateAdj: 0,
          })
          if (typeof ro.price === 'number') ro.price -= 1.125
        })
      })
    }
    ...
    const dscrCodeSent = isDSCRRequest ? mapDSCRRatio(formData.dscrRatio, parseFloat(formData.dscrValue) || 0, formData.dscrManualInput) : null
    ...
    return res.json({
      ...
      data: {
        ...
        debugSentValues: {
          ...
          dscrManualInput: formData.dscrManualInput || null,
        ...
        globalAdjustments: result.globalAdjustments
          ? result.globalAdjustments.filter((adj: any) => {
              const desc = (adj.description || '').toUpperCase()
              // Keep DSCR ratio tier adjustments — they are real LLPAs
              if (!isDSCRRequest && desc.includes('DSCR')) return false
              if (!isBankStmtRequest && desc.includes('BANK ST')) return false
              if (formData.loanPurpose === 'refinance' && desc.includes('CASHOUT')) return false
              return true
