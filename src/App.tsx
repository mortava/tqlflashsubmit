                          ...
                          return (
                            <div key={idx} className={`rounded-xl overflow-hidden border transition-all bg-white ${isSelectedPrepay ? 'border-slate-900 shadow-[0_1px_3px_rgba(0,0,0,0.04)]' : isExpanded ? 'border-[#D1D5DB] shadow-[0_1px_3px_rgba(0,0,0,0.04)]' : 'border-slate-200'}`}>
                              ...
                              {isExpanded && filteredRateOptions.length > 0 && (() => {
                                const optionsToShow = lockedUnlocked
                                  ? filteredRateOptions
                                  : (bestRate ? [bestRate] : [])
                                const hiddenCount = Math.max(0, filteredRateOptions.length - optionsToShow.length)
                                return (
                                <div className="border-t tql-border-steel">
                                  ...
                                  {/* ── Locked Options passcode gate ── */}
                                  {hiddenCount > 0 && (
                                    <div className="px-4 py-3 border-t tql-border-steel tql-bg-card">
                                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                                        <div className="flex items-center gap-2 min-w-0">
                                          <span className="inline-flex items-center justify-center w-5 h-5 rounded tql-bg-teal text-white text-[10px] font-bold">{hiddenCount}</span>
                                          <span className="text-[11px] font-semibold tql-text-slate tracking-wide">
                                            Locked Options <span className="tql-text-muted font-normal">&middot; final pricing 99.000 – 101.750</span>
                                          </span>
                                        </div>
                                        {lockedProgram !== programName ? (
                                          <button
                                            type="button"
                                            onClick={(e) => { e.stopPropagation(); setLockedProgram(programName); setLockedPasscodeInput(''); setLockedPasscodeError(false) }}
                                            className="inline-flex items-center gap-1.5 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-white tql-bg-teal rounded-md transition-colors"
                                          >
                                            Unlock
                                          </button>
                                        ) : (
                                          <form
                                            onSubmit={(e) => {
                                              e.preventDefault()
                                              if (lockedPasscodeInput.trim().toLowerCase() === LOCKED_OPTIONS_PASSCODE) {
                                                setLockedUnlocked(true)
                                                setLockedProgram(null)
                                                setLockedPasscodeInput('')
                                                setLockedPasscodeError(false)
                                              } else {
                                                setLockedPasscodeError(true)
                                              }
                                            }}
                                          ...
                                          >
                                            <input
                                              ...
                                              value={lockedPasscodeInput}
                                              onChange={(e) => { setLockedPasscodeInput(e.target.value); setLockedPasscodeError(false) }}
                                              className={`px-2.5 py-1 text-[11px] rounded-md border bg-white tql-text-primary w-28 sm:w-32 outline-none tql-ring-teal ${lockedPasscodeError ? 'border-[#EF4444]' : 'tql-border-steel'}`}
                                            />
                                            <button type="submit" className="inline-flex items-center px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-white tql-bg-teal rounded-md">
                                              Enter
                                            </button>
                                            <button type="button" onClick={() => { setLockedProgram(null); setLockedPasscodeInput(''); setLockedPasscodeError(false) }} className="p-1 tql-text-muted">
                                              <X className="w-3.5 h-3.5" />
                                            </button>
                                          </form>
                                        )}
                                      </div>
                                      {lockedProgram === programName && lockedPasscodeError && (
                                        <div className="mt-1.5 text-[10px] text-[#EF4444]">Incorrect passcode</div>
                                      )}
                                    </div>
                                  )}
                                  {lockedUnlocked && filteredRateOptions.length > 1 && (
                                    <div className="px-4 py-2 border-t tql-border-steel tql-bg-card flex items-center justify-between">
                                      <span className="text-[10px] font-semibold tql-text-teal uppercase tracking-widest">All pricing unlocked · {filteredRateOptions.length} options</span>
                                      <button type="button" onClick={() => setLockedUnlocked(false)} className="text-[10px] font-semibold tql-text-muted underline underline-offset-2">
                                        Re-lock
                                      </button>
                                    </div>
                                  )}
